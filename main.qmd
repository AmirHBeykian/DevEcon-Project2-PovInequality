...
Calculate the weighted household size.
...
```{r}
# Keep only one row for each household and it's urbainicity
head_demo <- demog |>
    filter(rel == 1) |>
    select(key, urban)

# Add urbanicity status to hh_size table
heis <- heis |>
    left_join(hh_size, by = "key") |>
    # Calculate weighted per capita expenditure
    mutate(expd_pc = total_expd / m_weight) |>
    left_join(head_demo, by = "key")
...
```{r}
# Generate sum of weigths for each household.
hh_size <- demog |>
    select(key, rel, age) |>
    mutate(m_weight = case_when(
        rel == 1 ~ 1,
...
heis <- heis |>
    ...
    mutate(pov_line = if_else(urban == "1", 152406849, 149130135))
...
```{r}
# Calculate total household education
hh_educations <- demog |>
    group_by(key) |>
    summarize(sum_educ = sum(educ, na.rm = TRUE))

# Calculate weighted household memebers
hh_size <- demog |>
    select(key, rel, age) |>
    mutate(m_weight = case_when(
        rel == 1 ~ 1,
...
top_1_percent_income_share <- heis |>
  ...
  summarize(total_income = sum(total_inc),
...
```{r}
# ohio_pop <- read.csv(file = "Data/ohio_pop.csv")
# ohio_pop <- as_tibble(ohio_pop) |>
...
# ohioShapesLeaflet <- merge(x = iran_shape, y = ohio_pop, by.x = "namelsad", by.y = "name", all.x = TRUE)

pal <- colorNumeric(
    palette = "Blues",
    domain = iran_shape$population
)
```
...
server <- function(input, output, session) {
    ...
    output$ohio <- renderLeaflet({
        leaflet() %>%
            addTiles() %>% 
            addPolygons(data = iran_shape,
                        stroke = FALSE,
                        smoothFactor = 0.2, 
                        fillOpacity = 1,
                        color = ~pal(population),
                        label = ~paste0(Name_12, ": ", population)
            )
    })
}
```
...
demog <- demog |>
    filter(rel == 1) |>
    select(key, urban, rel, age, educ, lit, weight_int) |>
    left_join(hh_educations, by = "key")
